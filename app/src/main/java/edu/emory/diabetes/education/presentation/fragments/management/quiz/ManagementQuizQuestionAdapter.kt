...
class ManagementQuizQuestionAdapter @Inject constructor(
    private val viewModel: ManagementQuizQuestionViewModel,
                ...
                true -> {
                    ...
                    bind.constraintLayoutItem.apply {
                        asyncListDiffer.currentList.forEach { _ ->
                            bind.checkBox.buttonTintList = ColorStateList.valueOf(
    ...
    fun setAnswers(question: Question, quizId: Int, submittedAns: List<String>) {
        ...
        this@ManagementQuizQuestionAdapter.quizId = quizId
            ...
            if (Utils.hasSomeAnswers(submittedAns, question.answers, 5)) {
                ...
                if (wrongChoice.isEmpty()) {
                    ...
                    listener?.onSubmitResultState(
                        Utils.RESULTS_ON_SUBMIT.HAS_SOME_CORRECT,
                        "${
                            answers.sorted().joinToString(separator = ", ")
                        } is not right. Please choose again.",
                ...
                for (i in wrongChoice.indices) {
                    ...
                    question.choices.forEach { choice ->
                        if (choice.id == wrongChoice.elementAt(i)) wrongChoiceIndexes.add(
                            AnswerData(
                                wrongChoice.elementAt(i),
                                count
                            )
                        )
                        count++
                ...
                if (trialCount > 0) {
                    if (this.wrongChoiceIndexes.size != 0) this.wrongChoiceIndexes =
                        wrongChoiceIndexes
                } else {
